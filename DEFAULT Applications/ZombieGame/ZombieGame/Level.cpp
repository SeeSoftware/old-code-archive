...
Level::Level(const std::string& fileName) {
    ...
    // Error checking
    if (file.fail()) {
		GE2D::fatalError("Failed to open " + fileName);
    }
            ...
            switch (tile) {
                ...
                case 'R':
                    _spriteBatch.draw(destRect,
                                      uvRect,
						GE2D::ResourceManager::getTexture("Textures/red_bricks.png").id,
                                      0.0f,
                                      whiteColor);      
                    break;
                case 'G':
                    _spriteBatch.draw(destRect,
                                      uvRect,
						GE2D::ResourceManager::getTexture("Textures/glass.png").id,
                                      0.0f,
                                      whiteColor);
                    break;
                case 'L':
                    _spriteBatch.draw(destRect,
                                      uvRect,
						GE2D::ResourceManager::getTexture("Textures/light_bricks.png").id,
                                      0.0f,
                                      whiteColor);
                    break;
                case '@':
