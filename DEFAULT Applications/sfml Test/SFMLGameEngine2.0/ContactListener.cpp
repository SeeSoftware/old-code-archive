...
namespace GESFML
{
	...
	void ContactListener::BeginContact(b2Contact* contact)
	{
		...
		void ContactListener::EndContact(b2Contact* contact)
		{
			...
			if (_batch->getPhysicSpriteCollisionIdsBatch().count(idA) > 0)
			{
				for (int i = 0; i < static_cast<int>(_batch->getPhysicSpriteCollisionIdsBatch()[idA].size()); i++)
				{
			...
			{
				for (int i = 0; i < static_cast<int>(_batch->getPhysicSpriteCollisionIdsBatch()[idB].size()); i++)
				{
