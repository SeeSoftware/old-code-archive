...
namespace GESFML
{
	...
	void SpriteBatch::createPhysicSpriteBox(long long id, float x, float y, float w, float h, bool dynamic, float density, float friction, float bounce, const sf::Texture &texture, sf::Color color, float rotation)
	{
		...
		bodyDef.angle = rotation * 3.141592653589793 / 180.0;
		if (dynamic)
	...
	void SpriteBatch::createRevoluteJoint(long long firstBodyid, long long secondBodyid, b2Vec2 localAnchorA, b2Vec2 localAnchorB, bool collideConnected, float baseangle)
	{
		...
		revoluteJointDef.referenceAngle = baseangle* 3.141592653589793 / 180.0;
	...
	void SpriteBatch::createRevoluteJointAngLimit(long long firstBodyid, long long secondBodyid, b2Vec2 localAnchorA, b2Vec2 localAnchorB, bool collideConnected, float baseangle, float upperAngleLimit, float lowerAngleLimit)
	{
		...
		revoluteJointDef.upperAngle = upperAngleLimit* 3.141592653589793 / 180.0;
		revoluteJointDef.lowerAngle = lowerAngleLimit* 3.141592653589793 / 180.0;
	...
	void SpriteBatch::setSpriteTexture(long long id, const sf::Texture &texture)
	{
		if (_spriteBatch.count(id) > 0)
		{
			_spriteBatch[id].setTexture(texture);
			if (_shadowBatch.count(id) > 0)
			{
				_shadowBatch[id].setTexture(texture);
			}
		}
	...
	void SpriteBatch::setSpriteOriginMiddle(long long id)
	{
		if (_spriteBatch.count(id) > 0)
		{
			_spriteBatch[id].setOrigin(getSpriteTexture(id)->getSize().x / 2, getSpriteTexture(id)->getSize().y / 2);
			if (_shadowBatch.count(id) > 0)
			{
				_shadowBatch[id].setOrigin(getSpriteShadowTexture(id)->getSize().x / 2, getSpriteShadowTexture(id)->getSize().y / 2);
			}
		}
	}
	...
	void SpriteBatch::restoreSpriteShadowTexture(long long id)
	{
		...
		if (_shadowBatch.count(id) > 0)
		{
			_shadowBatch[id].setTexture(*tmp);
		}
	}
...
/*	void SpriteBatch::replaceSprites(long long idfrom, long long idto)
	...
	bool SpriteBatch::testPointSprite(long long id, float x, float y)
	{
		if (_spriteBatch.count(id) > 0)
		{
			if (x > getSpritePosition(id).x){
				if (y > getSpritePosition(id).y)
				{
					...
					{
						if (y < getSpritePosition(id).x + getSpriteSize(id).y)
						{
							return true;
						}
					}
	...
	long long SpriteBatch::textPointSpriteId(long long id, float x, float y)
	{
		for (auto x2 : _spriteBatch)
		{
			if (testPointSprite(x2.first, x ,y))
			{
				return x2.first;
			}
		}
	...
	//---------------------------------------------------------------
	void SpriteBatch::createText(long long id, float x, float y, float wScale, float hScale, float characterSize, sf::Font &font, std::string text, sf::Color color, float rotation)
	{
		sf::Text tmp;
		tmp.setPosition(x, y);
	...
	void SpriteBatch::setTextCharacterSize(long long id, float size)
	{
		if (_textBatch.count(id) > 0)
		{
	...
	float SpriteBatch::getTextCharacterSize(long long id)
	{
		if (_textBatch.count(id) > 0)
		{
	...
	void SpriteBatch::setPhysicSpriteTexture(long long id, const sf::Texture &texture)
	{
		if (_physicSpriteBatch.count(id) > 0)
		{
			_physicSpriteBatch[id].setTexture(texture);
		}
	}
	...
	void SpriteBatch::setPhysicSpriteOriginMiddle(long long id)
	{
		if (_physicSpriteBatch.count(id) > 0)
		{
			_physicSpriteBatch[id].setOrigin(getPhysicSpriteTexture(id)->getSize().x / 2, getPhysicSpriteTexture(id)->getSize().y / 2);
			if (_physicShadowBatch.count(id) > 0)
			{
				_physicShadowBatch[id].setOrigin(getPhysicSpriteShadowTexture(id)->getSize().x / 2, getPhysicSpriteShadowTexture(id)->getSize().y / 2);
			}
		}
	}
	...
	void SpriteBatch::setPhysicSpriteRotation(long long id, float rotation)
	{
		if (_bodyBatch.count(id) > 0)
		{
			...
			_bodyBatch[id]->SetTransform(_bodyBatch[id]->GetPosition(), rotation* 3.141592653589793 / 180.0);
		}
	}
	...
	void SpriteBatch::setPhysicSpriteShadowTexture(long long id, const sf::Texture &texture)
	{
		if (_physicShadowBatch.count(id) > 0)
		{
			_physicShadowBatch[id].setTexture(texture);
		}
	}
	...
	void SpriteBatch::restorePhysicSpriteShadowTexture(long long id)
	{
		...
		if (_physicShadowBatch.count(id) > 0)
		{
			_physicShadowBatch[id].setTexture(*tmp);
		}
	}
...
/*	void SpriteBatch::replacePhysicSprites(long long idfrom, long long idto)
	...
	void SpriteBatch::setPhysicSpriteAngularVelocity(long long id, float angVelocity)
	{
		if (_bodyBatch.count(id) > 0)
		{
			_bodyBatch[id]->SetAngularVelocity(angVelocity* 3.141592653589793 / 180.0);
		}
	}
	...
	void SpriteBatch::addPhysicSpriteAngularVelocity(long long id, float angVelocity)
	{
		if (_bodyBatch.count(id) > 0)
		{
			_bodyBatch[id]->SetAngularVelocity(_bodyBatch[id]->GetAngularVelocity()*180 / 3.141592653589793 + angVelocity* 3.141592653589793 / 180.0);
		}
	}
	...
	void SpriteBatch::addPhysicSpriteRotation(long long id, float rotation)
	{
		if (_bodyBatch.count(id) > 0)
		{
			if (_physicSpriteBatch.count(id) > 0)
			{
				if (_physicShadowBatch.count(id) > 0)
				{
					_physicShadowBatch[id].setRotation(_physicShadowBatch[id].getRotation() + rotation* 3.141592653589793 / 180.0);
				}
				_physicSpriteBatch[id].setRotation(_physicSpriteBatch[id].getRotation() + rotation* 3.141592653589793 / 180.0);
			}
			_bodyBatch[id]->SetTransform(_bodyBatch[id]->GetPosition(), rotation* 3.141592653589793 / 180.0);
		}
	}
		...
		if (_bodyBatch.count(id) > 0)
		{
			if (_physicSpriteBatch.count(id) > 0)
			{
				...
				if (_physicShadowBatch.count(id) > 0)
				{
					...
					_physicShadowBatch[id].setRotation(_bodyBatch[id]->GetAngle() *180.0 / 3.14159265);
				}
				_physicSpriteBatch[id].setRotation(_bodyBatch[id]->GetAngle() *180.0 / 3.14159265);
			}
		}
	}
	...
	std::map<long long, sf::Sprite>& SpriteBatch::getSpriteShadowBatch()
	{
		return _shadowBatch;
	}

	std::map<long long, sf::Sprite>& SpriteBatch::getSpriteBatch()
	{
		return _spriteBatch;
	}

	std::map<long long, sf::Sprite>& SpriteBatch::getPhysicSpriteShadowBatch()
	{
		return _physicShadowBatch;
	}

	std::map<long long, sf::Sprite>& SpriteBatch::getPhysicSpriteBatch()
	{
	...
	void SpriteBatch::applyAngularForcePhysicSprite(long long id, float force)
	{
		if (_bodyBatch.count(id) > 0)
		{
			_bodyBatch[id]->ApplyTorque(force* 3.141592653589793 / 180.0, true);
		}
	}
	...
	void SpriteBatch::applyAngularImpusePhysicSprite(long long id, float impulse)
	{
		if (_bodyBatch.count(id) > 0)
		{
			_bodyBatch[id]->ApplyAngularImpulse(impulse* 3.141592653589793 / 180.0, true);
		}
	}
