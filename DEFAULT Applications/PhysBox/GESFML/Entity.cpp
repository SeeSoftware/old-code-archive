...
namespace GESFML {
	...
	Entity::Entity(int x, int y, int w, int h, sf::Texture &texture, bool dyn, b2World *World) {
		initBox(x, y, w, h, texture, dyn, World);

	}

	void Entity::initBox(int x, int y, float w, float h, sf::Texture &texture, bool dyn, b2World *World) {
	...
	void Entity::initSphere(int x, int y, float w, float h, sf::Texture &texture, bool dyn, b2World *World) {
		_sprite.setTexture(texture);
		_sprite.setPosition(x, y);
		_sprite.setScale(w / texture.getSize().x, h / texture.getSize().y);
		_sprite.setOrigin(texture.getSize().x / 2, texture.getSize().y / 2);
	...
	void Entity::initBodyBox(int x, int y, float w, float h, bool dyn, b2World *world) {
		_dyn = dyn;
		_world = world;
